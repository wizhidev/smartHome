...
int log_test(void) {
    int count = 10;
    log_set_file("/Users/hwp/work/mySource/c_design/clibraries/test.log");
    log_set_level(LOG_LEVEL_DEBUG);
    log_set_syslog("127.0.0.1", 8888, SYSLOG_LOCAL0, "www.iqiyi.com", "test");

    char str[1024] = {0};
    while (count-- > 0) {
        sprintf(str, "%s###%s", __FILE__, __func__);
        log_debug(str);
    }
    log_close();
